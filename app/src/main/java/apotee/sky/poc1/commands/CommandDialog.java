...
import apotee.sky.poc1.ApiPermissionActivity;
...
public class CommandDialog extends AppCompatActivity {
    ...
    static class CounterInputMethod extends InputDialog<View> {
        ...
        View createWidgetView(AppCompatActivity activity) {
            View layout = View.inflate(activity, apotee.sky.poc1.R.layout.dialog_counter, null);
            counterLabel = layout.findViewById(apotee.sky.poc1.R.id.counterTextView);

            final Button incrementButton = layout.findViewById(apotee.sky.poc1.R.id.incrementButton);
            incrementButton.setOnClickListener(view -> increment());

            final Button decrementButton = layout.findViewById(apotee.sky.poc1.R.id.decrementButton);
            ...
            for (final ActivityManager.RunningAppProcessInfo processInfo : runningProcesses) {
                if (processInfo.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                    for (final String activeProcess : processInfo.pkgList) {
                        if (activeProcess.equals("apotee.sky.poc1")) {
                            return true;
                        }
                    }
                }
    ...
    static class SpinnerInputMethod extends InputDialog<Spinner> {
        ...
        Spinner createWidgetView(AppCompatActivity activity) {
            ...
            final ArrayAdapter<String> adapter = new ArrayAdapter<>(activity, apotee.sky.poc1.R.layout.spinner_item, items);
