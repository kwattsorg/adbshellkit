...
public class CommandDialog extends AppCompatActivity {
    ...
    static class CounterInputMethod extends InputDialog<View> {
        ...
        View createWidgetView(AppCompatActivity activity) {
            View layout = View.inflate(activity, net.kwatts.android.droidcommandpro.R.layout.dialog_counter, null);
            counterLabel = layout.findViewById(net.kwatts.android.droidcommandpro.R.id.counterTextView);

            final Button incrementButton = layout.findViewById(net.kwatts.android.droidcommandpro.R.id.incrementButton);
            incrementButton.setOnClickListener(view -> increment());

            final Button decrementButton = layout.findViewById(net.kwatts.android.droidcommandpro.R.id.decrementButton);
            ...
            for (final ActivityManager.RunningAppProcessInfo processInfo : runningProcesses) {
                if (processInfo.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                    for (final String activeProcess : processInfo.pkgList) {
                        if (activeProcess.equals("net.kwatts.android.droidcommandpro")) {
                            return true;
                        }
                    }
                }
    ...
    static class SpinnerInputMethod extends InputDialog<Spinner> {
        ...
        Spinner createWidgetView(AppCompatActivity activity) {
            ...
            final ArrayAdapter<String> adapter = new ArrayAdapter<>(activity, net.kwatts.android.droidcommandpro.R.layout.spinner_item, items);
    ...
    static class SpeechInputMethod extends InputDialog<TextView> {
        ...
        public void create(final AppCompatActivity activity, final InputResultListener resultListener) {
            ...
            if (!ApiPermissionActivity.checkAndRequestPermissions(activity, activity.getIntent(), Manifest.permission.RECORD_AUDIO)) {
                activity.finish();
            }
